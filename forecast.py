...
class Forecaster:
    ...
    def predict(self, callback):
        data, interval = ForexApi.get_historical_rates(self.pairName, self.interval, self.inputShape[1])
        input_data = np.array(data['close'])
        input_data = self.scaler.transform(input_data.reshape(-1, 1))
        predictions = self.model.predict(input_data.reshape(1, self.inputShape[1], 1))
        ...
        for i in range(1, len(predictions[0]) + 1):
            if interval == "1h":
                ...
                dates.append(last_tick + timedelta(days=i))
        predictions = self.scaler.inverse_transform(predictions.reshape(-1, 1))
        callback(data, {'dates': dates, 'values': predictions}, interval)
